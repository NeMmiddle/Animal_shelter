...
from cats.service import (add_photos_for_the_cat, create_cat_with_photos,
                          delete_cat, delete_photo, get_cat, get_cats,
                          update_cat)
...
async def update_cat_by_id(cat_id: int, cat_update: CatUpdate, db=Depends(get_db)):
    """
    We can update the fields of an existing cat
    and the name of its photos folder in Google Drive.
    """
    return await update_cat(db, cat_id, cat_update)
...
async def delete_cat_photo_handler(cat_id: int, photo_id: int, db=Depends(get_db)):
    ...
    """
    await delete_photo(db=db, photo_id=photo_id, cat_id=cat_id)
